...
public class AudioManager : SingletonMonoBehaviour<AudioManager>
{
	...
	private const float SE_VOLUME_DEFULT  = 0.8f;

	//チャンネル指定
	private const int SE_SOURCE_NUM = 4;//SEのチャンネルの数.
	private const int PLAY_ONE_SHOT_SE_CH = 0;//one_shot_seをするときのチャンネル.
	...
	private void Awake ()
	{
		...
		for(int i = 0; i < audioSourceArray.Length; i++){
			...
			if(i == 0){
				...
				audioSourceArray [i].volume = SE_VOLUME_DEFULT;
			}
	...
	/// </summary>
	public void PlaySE (string seName, int ch = -1, float vol = SE_VOLUME_DEFULT)
	{
		...
		if (ch == -1) {
			GetAudioSource (PLAY_ONE_SHOT_SE_CH).PlayOneShot(_seDic [seName] as AudioClip);
			return;
		}
	...
	public AudioSource GetAudioSource(int ch = -1){
		if(ch == -1){
			...
			return _seSourceList [PLAY_ONE_SHOT_SE_CH];//全て再生中ならワンショット用を返す.
		}
