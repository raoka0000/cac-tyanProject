...
public class LAppModel :L2DBaseModel
{
    ...
    private ModelSetting modelSetting = null;	// モデルファイルやモーションの定義
	  ...
	  private Matrix4x4 matrixHitArea; // ヒットエリア描画用

    private String idleGroupName = LAppDefine.MOTION_GROUP_IDLE;
    ...
    public void Update()
    {
        ...
        if (mainMotionManager.isFinished())
        {
            ...
            StartRandomMotion(idleGroupName, LAppDefine.PRIORITY_IDLE);
        }
        //-----------------------------------------------------------------
    ...
    public void ChangeIdleMotion(string group){
      idleGroupName = group;
    }
